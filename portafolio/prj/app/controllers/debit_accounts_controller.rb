...
class DebitAccountsController < ApplicationController
  before_action :set_user
  before_action :require_permission
  before_action :require_ownership_permission, only: %i[show]

  # GET /users/:user_id/debit_accounts/new
  def new
    @account = DebitAccount.new
  end

  # POST /users/:user_id/debit_accounts
  def create
    @account = DebitAccount.new(account_params)
    @account.user = @user
    if @account.save
  ...
  def account_params
    params.require(:debit_account).permit(:currency, :liquid_balance)
  end
  ...
  def require_ownership_permission
    @account = DebitAccount.find(params[:id])
    is_admin = user_signed_in? && current_user.admin
